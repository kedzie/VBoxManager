...
final class Util {
    ...
    public static TypeMirror getApplicationSupertype(TypeElement type) {
        ...
        return isPlatformType(supertype.toString()) ? null : supertype;
    }
    ...
    /** Returns a string for {@code type}. Primitive types are always boxed. */
    public static String typeToString(TypeMirror type, boolean boxPrimitive) {
        if(type.getKind()==TypeKind.VOID)
            return "void";
        StringBuilder result = new StringBuilder();
        typeToString(type, result, '.', boxPrimitive);
...
//    }
    ...
    public static void typeToString(final TypeMirror type, final StringBuilder result,
                                    final char innerClassSeparator, final boolean boxPrimitive) {
        type.accept(new SimpleTypeVisitor6<Void, Void>() {
            @Override public Void visitDeclared(DeclaredType declaredType, Void v) {
                ...
                if (!typeArguments.isEmpty()) {
                    ...
                    for (int i = 0; i < typeArguments.size(); i++) {
                        ...
                        typeToString(typeArguments.get(i), result, innerClassSeparator, boxPrimitive);
                    }
                    result.append(">");
                }
                return null;
            ...
            @Override public Void visitPrimitive(PrimitiveType primitiveType, Void v) {
                if(boxPrimitive)
                    result.append(box(primitiveType).toString());
                else
            ...
            @Override public Void visitArray(ArrayType arrayType, Void v) {
                ...
                if (type instanceof PrimitiveType) {
                    ...
                    typeToString(arrayType.getComponentType(), result, innerClassSeparator, boxPrimitive);
                }
                result.append("[]");
                return null;
            }
    ...
    private static final AnnotationValueVisitor<Object, Void> VALUE_EXTRACTOR =
            new SimpleAnnotationValueVisitor6<Object, Void>() {
                ...
                public Object visitEnumConstant(VariableElement c, Void aVoid) {
                    try {
                        Class clazz = Class.forName(typeToString(c.asType()));
    ...
    public static Map<String, Object> getAnnotation(Class<?> annotationType, Element element) {
        for (AnnotationMirror annotation : element.getAnnotationMirrors()) {
            ...
            for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> e
                ...
                if (defaultValue!=null && !lenientIsInstance(defaultValue.getClass(), value)) {
