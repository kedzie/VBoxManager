 ...
 * <p>Any caller wishing to manipulate a virtual machine needs to create a session object first, which lives in its own process space. Such session objects are then associated with {@link IMachine}<b></b> 
 * objects living in the VirtualBox server process to coordinate such changes.<p>
 ...
 * Whereas multiple sessions may control a running virtual machine, only one process can obtain a write lock on the machine to prevent conflicting changes. A write lock is also needed if a process 
 * wants to actually run a virtual machine in its own context, such as the VirtualBox GUI or VBoxHeadless front-ends. They must also lock a machine for their own sessions before they are allowed to power up the virtual machine.<p>
 ...
 * <li>To start a VM using one of the existing VirtualBox front-ends (e.g. the VirtualBox GUI or VBoxHeadless), one would use {@link IMachine#launchVMProcess}<b></b>, which also takes a session 
 * object as its first parameter. This session then identifies the caller and lets the caller control the started machine (for example, pause machine execution or power it down) as well as be notified about machine execution state changes.  </li>
 * </ul>
 ...
 * <ul>
 * <li>When using the COM API directly, an object of the Session class from the VirtualBox type library needs to be created. In regular COM C++ client code, this can be done by calling <code>createLocalObject()</code>, 
 * a standard COM API. This object will then act as a local session object in further calls to open a session.  </li>
...
public interface ISession extends IManagedObjectRef, Parcelable {
	...
	public static final Parcelable.Creator<ISession> CREATOR = new Parcelable.Creator<ISession>() {
		public ISession createFromParcel(Parcel in) {
			...
			return vmgr.getProxy(ISession.class, id, cache);
		}
