...
public class DictionaryOpenHelper extends SQLiteOpenHelper {
	private static final String TAG = DictionaryOpenHelper.class.getName();
    ...
    private static final String TABLE_CREATE ="CREATE TABLE " + TABLE_NAME + " (" + ROW_ID + " INTEGER PRIMARY KEY," + ROW_HOST + " TEXT, " + ROW_PORT + " INTEGER);";
    ...
    public List<Server> getServers() {
    	Cursor c = getReadableDatabase().query(TABLE_NAME, new String[] { ROW_ID, ROW_HOST, ROW_PORT }, null, null, null, null, null);
    	List<Server> ret = new ArrayList<Server>();
    	c.moveToFirst();
    	for(int i=0; i<c.getCount(); i++) {
    		Server s = new Server(c.getString(c.getColumnIndex(ROW_HOST)), c.getInt(c.getColumnIndex(ROW_PORT)));
    		s.setId(c.getLong(c.getColumnIndex(ROW_ID)));
    		ret.add(s);
    		c.moveToNext();
    	}
    	return ret;
